...
erDiagram
  ...
  Items ||--o{ ItemStocks : "has"
  Sites ||--o{ ItemStocks : "manage"
  ...
  %% 商品在庫情報を管理するテーブル
  ItemStocks {
    integer id PK "AUTO_INCREMENT" %% 在庫のユニークなID
    reference item_id FK "NOT_NULL: Items.id" %% 商品ID
    reference site_id FK "NOT_NULL: Sites.id" %% サイトID
    boolean is_infinite_stock "NOT_NULL: default: false" %% 無限大在庫フラグ
    integer stock_quantity "default: 0" %% 在庫数
    integer safety_stock_quantity "default: 0" %% 安全在庫数
    datetime created_at "NOT_NULL" %% 作成日時
    datetime updated_at %% 更新日時
  }
